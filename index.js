...
function logArrayElements(arr) {
  ...
  if (!Array.isArray(arr)) {
    // Перевіряємо, чи arr є масивом
    console.error("Аргумент має бути масивом!"); // Виводимо помилку в консоль
    return null; // Повертаємо null для позначення неможливості обробки вхідних даних
  }

  arr.forEach((element, index) => {
    console.log(`Елемент ${index}: ${element}`); // Виводимо поточний елемент та його індекс в консоль
  });
}
...
function warnAboutMaxArrayLength(arr, maxLength) {
  ...
  if (arr.length > maxLength) {
    // Перевіряємо, чи довжина масиву перевищує максимальну допустиму довжину
    console.warn("Увага! Довжина масиву перевищує максимально допустиму!"); // Виводимо попередження в консоль
  } else {
    console.log("Довжина масиву не перевищує максимально допустиму."); // Виводимо повідомлення про немає проблем в консоль
  }
}
...
function compareArrays(arr1, arr2) {
  ...
  arr1.sort((a, b) => a - b); // Сортуємо перший масив за зростанням
  arr2.sort((a, b) => a - b); // Сортуємо другий масив за зростанням
...
function buildMatrix(size) {
  ...
  const matrix = []; // Створюємо порожній масив, який буде матрицею

  for (let i = 0; i < size; i++) {
    const row = []; // Створюємо порожній масив для рядка матриці

    for (let j = 0; j < size; j++) {
      const randomNumber = Math.floor(Math.random() * 10); // Генеруємо випадкове число від 0 до 9

      row.push(randomNumber); // Додаємо число до рядка
    }

    matrix.push(row); // Додаємо рядок до матриці
  }

  console.table(matrix); // Виводимо матрицю в консоль як таблицю
...
function traceBackward(n) {
  ...
  if (n > 1) {
    traceBackward(n - 1); // Викликаємо функцію traceBackward зменшену на 1
  } else {
    console.trace(); // Виводимо стек викликів, коли досягнемо 1
  }
}
...
function calculateTotalPrice(products) {
  ...
  let total = 0; // Змінна для збереження загальної вартості товарів

  for (let i = 0; i < products.length; i++) {
    total += products[i].price; // Додавання ціни товару до загальної вартості
  }
...
function countVowelsAndConsonants(word) {
  ...
  for (let letter of word) {
    letter = letter.toLowerCase(); // Перетворення літери на малу для порівняння
